...
- **Issue Context Brief**: 要求・制約の正本はIssue本文。担当Codex sessionは着手時にIssue本文と信頼できる最新 `ctx-brief` コメントを明示取得し、Issue番号とsnapshotが一致することを確かめる。Brief内のJev L1候補は実装時に読む資料への助言で、正しさ・無関係さ・許可の判定には使わない。コメントが存在するだけではsessionへの配達完了とみなさない
