...
6. `status:ready` を `status:in-progress` へ差し替え、**その issue 自身**にコメントで dispatch 先（session / agent / provider）と scope を記録する。束ねた場合は代表 issue にコメントし、他は代表へリンクする。**この着手のタイミング（レーンへの割り当て、または PR の Closes に載せた時点）で、対象 issue に現行 milestone を付与する**（編成時(操作 B 手順 5)の判断とは独立に、着手＝付与を機械的に行う。#2006）。**レーンが draft PR を作成した時点で、PR 自身にも現行 milestone を付与する**（issue 側だけでなく PR 側にも付いていると release notes 作成時の集計が楽になる。#2065）。**差し替えたら `pnpm ctx <N> --post` を実行してL1候補入りbrief（関連 PR / 触るファイル / 保護対象の要否 / 決定ログ / 次の一手）を issue コメントへ置く**（worker は issue URL しか受け取らないので、最初のturnより前に選別・圧縮済みのL0とJevのL1助言を届ける。このコマンドがL1を生成する通常の入口で、同じ入力なら既存候補を再利用し、同じコメントを更新する。品質評価は利用開始の条件にしない。`routing` skill）。**この dispatch コメントに DoD（完了の定義）を 1〜3 行で記載する**（「仕様には適合しているが意図とズレている」静かな失敗は着手時点の意図と突き合わせないと見つからない。束ねた場合は代表 issue のコメントへ一括で書く。突き合わせは PR レビュー時と、ズレを疑った時に行う。[#2273](https://github.com/Dayopt/dayopt/issues/2273)）
7. worker への指示は issue URL + 「最初に `pnpm ctx <N> --reuse-brief-l1` でIssue本文と最新Briefを確認し、本文の受け入れ条件と検証コマンドに従う」だけで済む状態にする。Briefが古い/無い/取得できない場合も報告してL0と一次資料で続ける。通常読取でJev APIは呼ばない。着手手順・PR 規約・報告テンプレート・検証原則はチップ prompt へ個別に書き下さず `AGENTS.md` §レーン運用 への参照 1 行で足りる
...
```markdown
## 背景 — 解決する問題と期待する変化。確認済み事実と未確認の仮説を分け、関連 issue / docs / 過去 PR を参照

## やること — Goal、Scope / Non-goals、受け入れ条件、未解決事項を書く。**受け入れ条件（何ができたら完了か）を 1 行以上**。対象 path や番号付き手順は必要な制約だけに絞り、未確認の実装方法を確定事項にしない。外部リサーチが要る時は「外部リサーチ: <問い / 必要な一次資料 / 出力形式>」を 1 行書き、結果が前提なら `status:blocked`。provider は固定せず、結果を issue コメントへ残す

## 注意 — 触らない範囲、既存の権限境界、外部作用、必要な停止・復帰条件、関連 skill。該当なしの場合は理由を書く

## 検証 — 受け入れ条件を確かめるコマンド（**そのまま実行できる形**）または具体的な確認手順。必要な検証と実行済みの結果を分け、未実行・失敗も記録する

## 期待出力（該当時のみ）— 返してほしい形式。分類軸、判断ごとの証拠水準、撤退・rollback 条件の明示要求
```

GitHub Issue Forms は入力案内であり、この4節の意味の正本は本テンプレートとする。Markdown / API 経由で起票する場合も同じ契約を使う。L0 は Markdown heading の段数や短い説明文に関わらず4節を読み、空欄・TBD・placeholder、根拠のない「該当なし」、検証コマンドの不足を示す。根拠付きの「該当なし」は有効な回答として扱う。この形式確認は事実の正しさ・安全性や権限を保証せず、ready / merge の許可にもならない。
...
Intakeでは観測した事実・解釈・未検証の仮説を分け、安く確かめられる事実は起票前に確認する。関連issueを同じ変更レーンへ束ねる場合も、各issueの受け入れ条件・状態・完了を個別に追跡する。別issueに分けるのは独立した対象・受け入れ条件・時期を持つ仕事に限る。
...
1. `gh search issues` で既存 issue と後継を open / closed の両方から検索し、重複を確認する
