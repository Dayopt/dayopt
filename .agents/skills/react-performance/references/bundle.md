...
上流の原文を Dayopt 向けに抜粋・再構成したもので、公式原文そのままではない。

**最重要の Dayopt 差分**: 上流の `bundle-barrel-imports` は「barrel を避けて実体から import する」と書くが、これは **node_modules の第三者 package に対する規則**として読む。Dayopt の feature 間 import は barrel 経由が必須で、deep import は `pnpm lint:boundaries` が機械的に禁止している。内部境界を性能を理由に壊さない。

---

## Avoid Barrel File Imports

`bundle-barrel-imports` — impact: CRITICAL（上流表記: 200-800ms import cost, slow builds）
...
`bundle-dynamic-imports` — impact: CRITICAL（上流表記: directly affects TTI and LCP）
...
`bundle-defer-third-party` — impact: MEDIUM（上流表記: loads after hydration）
...
`bundle-conditional` — impact: HIGH（上流表記: loads large data only when needed）
...
`bundle-preload` — impact: MEDIUM（上流表記: reduces perceived latency）
...
`bundle-analyzable-paths` — impact: HIGH（上流表記: avoids accidental broad bundles and file traces）
...
> **Dayopt 注記**: 動的な import path の回避は第三者 package と生成コードに適用する。feature barrel の再 export を「静的解析しにくい」という理由で外さない。
