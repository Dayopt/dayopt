...
- runtime で read-only と repository scope を同時に強制できる adapter がないため、大量の読み取り調査も主担当が行う。native delegation は read-only 経路には使わず、User が明示した write / browser task は既存の非重複 scope・authority 契約で別途扱う。両方を実測できる adapter が追加された場合だけ、Luna / Haiku の候補を再評価する
