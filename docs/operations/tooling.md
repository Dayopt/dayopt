...
- `pnpm branch:finish` はlinked worktreeなら従来どおり削除する。Cloud等の通常checkoutは未保存差分がなく、対象branchがorigin/mainへ到達していることを確認してdetachし、ディレクトリを保持する。別branchのcheckoutは切り替えない。GitHubのmerge条件は実行場所で変えない
