...
CI の Secret は repo 単位ではなく、`main` だけから使える environment（`production-release` / `production-ops`）に置く（2026-09-14）。どの Secret をどの environment に置くかと同期手順は [Operations / Secrets](../secrets.md) §GitHub Secrets が正本。
...
| `RCLONE_CONFIG_SOURCE_*`（6 件）   | Storage backup export（nightly.yml）         | 1Password `ci/Supabase-StorageS3-backupsource` の replica。Supabase Storage S3 の読み取り。`storage-backup-export` job の step だけへ渡す                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `RCLONE_CONFIG_DEST_*`（6 件）     | Storage backup export（nightly.yml）         | 1Password `ci/Cloudflare-R2-storagebackup` の replica。R2 bucket への書き込み（`--max-delete` 上限付き sync）。同上                                                                                                                                                                                                                                                                                                                                                                                                                                              |

参照する workflow の無かった `SENTRY_AUTH_TOKEN`（2026-08-24 登録）は 2026-09-14 に User が削除した（`gh secret list` で不在を確認）。
...
Production replicaはProductが`human/sentry`、Webが`human/sentry-web`をmetadata / DSNのmasterとする。build tokenだけは`ci/sentry-release-token`を両projectへ同期する（#2085 で分離。`human/sentry-login` は GUI ログイン item で token を持たない）。
...
```bash
DAYOPT_CONFIRM_TARGET=<linked project ref> pnpm db:reset-linked:unsafe
```

`DAYOPT_CONFIRM_TARGET` が `supabase/.temp/project-ref`（link 先）と一致しないと reset しない（2026-09-14、`scripts/tasks/confirm-target.sh`）。
