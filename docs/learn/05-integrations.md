...
| 振る舞い    | 意味                                   | Dayopt の例                                                                                                                                                                                  |
| ----------- | -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| fail-closed | 確かめられないなら止める               | メールの suppression 照会が失敗したら送らない。Upstash が落ちたら MCP・AI クライアントの token 発行・本人確認（アカウント削除・メール変更）・問い合わせは 503 / SERVICE_UNAVAILABLE で止める |
| fail-open   | 確かめられないなら通す（可用性を優先） | Upstash が落ちたら、**画面の tRPC だけ**は rate limit をメモリ上へ退避して通す（認証前の IP 単位は判定せず通す）。漏洩パスワード確認の失敗は通す                                             |
