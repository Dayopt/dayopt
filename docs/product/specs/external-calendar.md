...
- 2026-09-25 の読み取り確認では Production に active な接続が 1 件あり、`authority_fence_id` / `authority_epoch` はどちらも NULL、最終同期は約 35.8 日前だった。cron heartbeat は 8 件すべて新しかったが、接続の同期成功は確認できていない。新しい再接続導線はこの legacy 行を再認証で fence 付きに戻す設計で、反映と同一 Google account による再認証・同期確認が残っている
