-- Keep the existing subscription-only MCP contract until the operator enables
-- the new app-trial access model with a revision-checked control update.
BEGIN;
...
COMMENT ON COLUMN public.mcp_mutation_control.billing_enforced IS
  'When true, MCP writes accept active app trials in addition to paid subscription states.';

CREATE FUNCTION public.set_mcp_billing_enforcement_v1(
  p_billing_enforced BOOLEAN,
...
BEGIN
  ...
  IF p_billing_enforced IS NULL
    OR p_expected_revision IS NULL
    OR p_expected_revision < 0 THEN
    RAISE EXCEPTION 'Invalid MCP billing control input'
      USING ERRCODE = '22023';
  END IF;

  RETURN QUERY
  UPDATE public.mcp_mutation_control AS control
  SET billing_enforced = p_billing_enforced,
      revision = control.revision + 1,
      changed_at = pg_catalog.now()
  WHERE control.singleton_key = true
    AND control.revision = p_expected_revision
  RETURNING control.billing_enforced, control.revision, control.changed_at;
...
REVOKE ALL ON FUNCTION public.set_mcp_billing_enforcement_v1(BOOLEAN, BIGINT)
FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.set_mcp_billing_enforcement_v1(BOOLEAN, BIGINT)
TO service_role;

COMMENT ON FUNCTION public.set_mcp_billing_enforcement_v1(BOOLEAN, BIGINT) IS
  'CAS update for MCP app-trial enforcement. Callable only by service_role.';
...
BEGIN
  ...
  SELECT
    control.writes_enabled,
    control.billing_enforced,
    control.enabled_client_ids
  INTO v_writes_enabled, v_billing_enforced, v_enabled_client_ids
  FROM public.mcp_mutation_control AS control
  WHERE control.singleton_key = true
  FOR SHARE;
  ...
  SELECT
    ...
    profile.app_trial_started_at,
    profile.app_trial_ends_at,
    profile.app_trial_consumed_at
  INTO
  ...
  IF NOT v_billing_enforced THEN
    IF v_subscription_status <> ALL (ARRAY['active', 'trialing', 'past_due']::TEXT[]) THEN
      RAISE EXCEPTION 'MCP Pro entitlement is required'
        USING ERRCODE = 'DM005';
    END IF;
  ELSIF v_subscription_status <> ALL (ARRAY['active', 'trialing', 'past_due']::TEXT[])
    AND NOT COALESCE(
      v_trial_consumed_at IS NULL
      AND v_trial_started_at <= v_now
      AND v_now < v_trial_ends_at,
      false
    ) THEN
    RAISE EXCEPTION 'Active Dayopt access is required'
      USING ERRCODE = 'DM005';
