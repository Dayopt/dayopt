...
interface UseScrollTimeblockIntoViewOptions {
...
export function useScrollTimeblockIntoView({
  scrollContainerRef,
  hourHeight,
}: UseScrollTimeblockIntoViewOptions) {
  const isMobile = useMediaQuery(MEDIA_QUERIES.mobile);
  const inspectorTimeblockId = useTimeblockInspectorStore((s) => s.timeblockId);
  ...
  useEffect(() => {
    ...
    const scrollToSelection = () => {
      const timeblockEl = container.querySelector<HTMLElement>(
        `[data-timeblock-id="${inspectorTimeblockId}"]`,
      );
  ...
  }, [isMobile, inspectorIsOpen, inspectorTimeblockId, scrollContainerRef]);
