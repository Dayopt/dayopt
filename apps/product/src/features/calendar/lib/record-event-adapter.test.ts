...
describe('independent record projection', () => {
  it('preserves each independent record without deriving a paired difference', () => {
    const rows = [
      makeRow({ source: 'from_plan' }),
    ...
    expect(events.every((event) => !('diffMinutes' in event))).toBe(true);
  });
